...
from scigantic_bindingdb import cache
from scigantic_bindingdb.cache import resolve
...
def test_disabled_by_default_returns_s3_url():
    assert bindingdb.is_cache_enabled() is False
    assert resolve(_SMALL_KEY) == f"s3://scigantic-bindingdb/{_SMALL_KEY}"


def test_enable_cache_downloads_and_reuses(tmp_path):
    resolved_dir = bindingdb.enable_cache(cache_dir=str(tmp_path))
    try:
        assert bindingdb.is_cache_enabled() is True
        assert resolved_dir == tmp_path
        assert bindingdb.cache_dir() == tmp_path
...
def test_cached_bridge_matches_uncached(tmp_path):
    ...
    # Filtered to one known row rather than a bare LIMIT: a plain LIMIT with
    # no ORDER BY on a joined query isn't guaranteed to return the same rows
    # on repeated calls, which would make this comparison flaky for reasons
    # unrelated to caching.
    reactant_set_id = "50891275"
    uncached = bindingdb.chembl_bridge(reactant_set_id=reactant_set_id, with_names=False)

    bindingdb.enable_cache(cache_dir=str(tmp_path))
    try:
        cached = bindingdb.chembl_bridge(reactant_set_id=reactant_set_id, with_names=False)
        cached_file = tmp_path / "202608" / "derived" / "bindingdb_chembl_bridge.parquet"
        assert cached_file.exists()
    finally:
        bindingdb.disable_cache()

    assert len(cached) == len(uncached) == 1
    assert cached["chembl_id"].iloc[0] == uncached["chembl_id"].iloc[0]
